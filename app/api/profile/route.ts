...
export async function GET(request: NextRequest) {
  try {
    ...
    const mentors = await db.collection("mentorships").aggregate([
    ...
    const mentees = await db.collection("mentorships").aggregate([
    ...
    const contributorProjects = await db.collection("contributorProjectMappings").find({ contributorId: new ObjectId(userId) }).toArray();
